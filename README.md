...
- **权限管控** ：核心管理指令（统计/抽取/队列维护等）仅限 Bot 管理员操作；`/统计详细记录` 与 `/上次统计详细` 开放给所有群成员查看。
...
插件的【核心管理指令】仅对 AstrBot 后台配置的 **管理员** 开放。如果你作为群主/管理员使用本插件时提示"无权限"，请按以下步骤配置：

1. 登录 AstrBot Web 管理面板
2. 依次进入：**配置** → **普通配置** → **平台配置** → **管理员ID**
3. 在列表中添加你的 **QQ号** 或 **用户ID**（多个用英文逗号分隔）
4. 保存后重启 AstrBot 使配置生效

> 💡 配置完成后即可使用所有【核心管理指令】：`/快速车统计`、`/结束统计`、`/快速抽取`、`/快速车提醒`、`/添加队列`、`/修改队列`、`/快速车添加`、`/快速车删除`、`/清空统计`。
>
> ℹ️ `/快速车帮助`、`/统计详细记录`、`/上次统计详细` 等查看类指令中，只有 `/快速车帮助` 仍需管理员权限；`/统计详细记录`、`/上次统计详细` 开放给所有群成员使用。
...
> **别名** ：`/统计详细记录` 可用 `/快速车详细`、`/统计详细`、`/快速车记录` 替代。

### 📌 管理员指令（需在 AstrBot 后台配置管理员ID）
